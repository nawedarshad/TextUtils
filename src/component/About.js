...
export default function About() { 
    ...
    let changeColorMode = () => {
        if (colorMode.color === "white") {
            setColorMode({color: "black", backgroundColor: "white"})
            setBtnMode("Enable Light Mode")
        } else {
            setColorMode({color: "white", backgroundColor: "#282c34", border: "0.5px solid white"})
            setBtnMode("Enable Dark Mode")
        }
    }
  return (
    <>
        <div className="accordion container my-3" id="accordionExample " style={colorMode}>
            <h1>About us</h1>
...
<button type='button' className='btn btn-primary my-3' onClick={changeColorMode}>{btnMode}</button>
</div>
    </>
  )
}
